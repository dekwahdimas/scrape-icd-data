...
full_code, main_source = [], []
new_main_code_list, new_main_text_list = [], []
...
for i, code in enumerate(sub_code, 0):
    for c in code:
        ...
        if main_code_list[i] in a_link: # check if main code is in href link (sub code)
            ...
            main_source.append(f"https://www.icd10data.com/{main_code_list[i]}") # get main source
            ...
            main_source.append("no_main_source")
...
filename = "parsed_data/parsed_1st_icd_10_cm_data.csv"
df.to_csv(filename, index=False, sep="|")
print(f"Saved as {filename}")

'''
The csv file output have sub_text with ... at the end of text,
because the text is too long so they replace it with ...
Use second_parse.py to complete the text.
'''
